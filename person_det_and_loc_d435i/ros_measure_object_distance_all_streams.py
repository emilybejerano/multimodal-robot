...
class PersonDetectionNode(Node):
    ...
    def timer_callback(self):
        ret, bgr_frame, depth_frame, ir_frame = self.rs.get_frame_stream()
        if not ret:
        ...
        # Draw object masks on the RGB frame
        masked_bgr_frame = self.mrcnn.draw_object_mask(bgr_frame)

        # Create a colored depth colormap for visualization
        ...
        # Overlay the detection info and movement command on the masked RGB frame
        cv2.putText(masked_bgr_frame, "RGB Stream", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        cv2.putText(masked_bgr_frame, self.detection_info, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        cv2.putText(masked_bgr_frame, self.movement_command, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)

        # Overlay descriptive text on the depth colormap and IR frame
        cv2.putText(depth_colormap, "Depth Stream", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(ir_frame, "Infrared Stream", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

        # Stack the RGB, depth colormap, and IR frames side by side
        combined_frame = np.hstack((masked_bgr_frame, depth_colormap, ir_frame))

        # Publish the image with object masks for visualization
        image_msg = self.bridge.cv2_to_imgmsg(masked_bgr_frame, encoding="bgr8")
        self.image_pub_.publish(image_msg)

        # Display the image with masks using OpenCV
        cv2.imshow("Person Detection", combined_frame)
